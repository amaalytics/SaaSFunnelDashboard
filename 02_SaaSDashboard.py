...
# ── Config: Creating the Dashboard ──
st.set_page_config(page_title = 'SaaS Funnel Dashboard', 
                   page_icon='🔎', 
                   layout='wide') 
...
# ── Header + KPIs ──
st.title("📊 SaaS Funnel Dashboard")
...
k1.metric("Total Leads", f"{len(fdf):,}") # F Strings handle the formatting
...
fig_sz = px.bar(sz, x=sz.index, 
                y="leads", 
                text="leads", 
                title="Lead Volume & Win Rate by Company Size")
fig_sz.add_scatter(x=sz.index, y=sz["win_rate"], 
                   mode="lines+markers+text",
                   text=sz["win_rate"].astype(str) + "%", 
...
# ── 3) Speed to Demo ──
st.subheader("⏱️ MQL-to-Demo Booking Duration")
...
summary = speed_df.groupby("speed").agg(
    Leads=("lead_id", "count"),
    Wins=("current_stage",
          lambda x: x.isin(["Closed Won", "Churned"]).sum())
)

summary["Win Rate %"] = (
    summary["Wins"] / summary["Leads"] * 100
).round(1)

st.subheader("Speed Performance Summary")
st.dataframe(summary, use_container_width=True)

# Histogram
fig_spd = px.histogram(
    speed_df,
    x="days_mql_to_demo_booked",
    color="current_stage",
    nbins=14,
    title="Distribution of Days MQL → Demo Booking",
    barmode="stack",
)

fig_spd.update_layout(
    height=400,
    xaxis_title="Days from MQL to Demo Booking",
...
fig_trend = px.bar(monthly, x="created_month", 
                   y="leads", 
                   color="period",
                   color_discrete_map={"Q4": "#F6C343", "Other": "#4C78A8"},
                   title="Leads per Month (Q4 highlighted)")

fig_trend.add_scatter(x=monthly["created_month"], 
                      y=monthly["win_rate"],
                      mode="lines+markers", 
                      name="Win Rate %", 
                      yaxis="y2",
